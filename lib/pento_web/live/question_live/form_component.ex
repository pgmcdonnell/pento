defmodule PentoWeb.QuestionLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:asked]} type="text" label="Asked" />
  ...
  def update(%{question: question} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Help.change_question(question))
     end)}
  end

  @impl true
  def handle_event("validate", %{"question" => question_params}, socket) do
    changeset = Help.change_question(socket.assigns.question, question_params)
  ...
  defp save_question(socket, :edit, question_params) do
    case Help.update_question(socket.assigns.question, question_params) do
      {:ok, question} ->
  ...
  defp save_question(socket, :new, question_params) do
    case Help.create_question(question_params) do
      {:ok, question} ->
        notify_parent({:saved, question})
